...
module List =
  struct
    ...
    let rec update xs k x =
      if xs=[] then
        raise (Error "list_update: index out of range")
      else
        if k=0 then
          x::(List.tl xs)
        else
    ...
    let remove1 x xs =
      let rec f rs = function
          [] -> raise (Error "list_remove1: not found")
        | y::ys -> if x = y then
    ...
    let insert xs k x =
      let rec f k rs xs =
        if k = 0 then
          ...
          match xs with
            [] -> raise (Error "list_insert: index out of range")
          | y::ys -> f (k-1) (y::rs) ys
      in
      if k < 0 then
        raise (Error "list_insert: index out of range")
    ...
    let remove_index xs k =
      let rec f k rs = function
          [] -> raise (Error "list_remove_index: index out of range")
        | x::xs ->
           if k = 0 then
